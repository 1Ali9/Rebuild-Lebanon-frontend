  ...
  const fetchManagedSpecialists = async () => {
    try {
      ...
      const specialistsData =
        response.data?.specialists || response.specialists || [];
  ...
  const getFilteredSpecialists = () => {
    ...
    if (filters.governorate) {
      filtered = filtered.filter(
        (specialist) => specialist.governorate === filters.governorate
      );
    }

    if (filters.district) {
      filtered = filtered.filter(
        (specialist) => specialist.district === filters.district
      );
    }

    if (filters.specialty) {
      filtered = filtered.filter(
        (specialist) => specialist.specialty === filters.specialty
      );
    }
          ...
          <div className="card-content">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3>
                  Specialist List ({filteredSpecialists.length} of{" "}
                  {managedSpecialists.length} specialists)
                </h3>
                <div className="flex gap-2">
                  ...
                  <span className="badge badge-unavailable">
                    ⏳ Pending:{" "}
                    {managedSpecialists.filter((s) => !s.isDone).length}
                  </span>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="dashboard-item">
                  <div className="dashboard-item-content text-center p-4">
                    ...
                    <p className="mb-4">
                      No specialists in your managed list yet
                    </p>
                      ...
                      <div className="dashboard-item-content">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              removeSpecialistFromManaged(specialist._id)
                            }
                          >
                            🗑️
                          </button>
